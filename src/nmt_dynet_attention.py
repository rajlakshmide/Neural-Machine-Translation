...
class nmt_dynet_attention:
    ...
    def __init__(self, src_vocab_size, tgt_vocab_size, src_word2idx, src_idx2word, tgt_word2idx, tgt_idx2word, word_d, gru_d, gru_layers):
        ...
        self.attention_B = self.model.add_parameters((self.gru_d, self.gru_d))
    ...
    def encode(self, src_sentence):
        ...
        for i in range(len(src_sentence)):
            hv = concatenate([forward_states[i], backward_states[-1-i]])
            hv_i.append(hv)
        return hv_i
    ...
    def attend(self, input_vectors, state):
        ...
        '''
        # YOUR IMPLEMENTATION GOES HERE
        #B = parameter(self.attention_B)
        beta_i_j = []
        m = len(input_vectors)
        #self.attention_w1 = self.model.add_parameters(m)
        #beta_i_j = parameter(self.attention_w1)
        #print (input_vectors.npvalue())
        for j in range(len((input_vectors))):
            beta_i_j.append(transpose(state)*input_vectors[j])
        #beta_i_j = beta_i_j.vec_value()
        beta_dy = vecInput(m)
        beta_dy.set([item.value() for item in beta_i_j])
        alpha_i_j = softmax(beta_dy)
        c_i=0
        for k in range(m):
            c_i+=(input_vectors[k]*alpha_i_j[k])
        #c_i = sum(alpha_i_j[k]*input_vectors[k] for k in range(len((input_vectors))))
        return c_i
    ...
    def get_loss(self, src_sentence, tgt_sentence):
        ...
        for word, next_word in zip(tgt_sentence, tgt_sentence[1:]):
            s = s.add_input(concatenate([self.target_embeddings[self.tgt_word2idx[word]], self.attend(encoding, h)]))
            h = s.output()
            probs = softmax(R*h + b)
    ...
    def generate(self, src_sentence):
        ...
        h = encoding[-1]
        while (True):
            s = s.add_input(concatenate([self.target_embeddings[self.tgt_word2idx[y]], self.attend(encoding, h)]))
            h = s.output()
            probs = softmax(R*h+b)
